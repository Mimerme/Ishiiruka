...
bool NetPlayLauncher::Host(const NetPlayHostConfig& config)
{
	...
	if (Smashladder::m_netplay) {
		netplay_client =
			new NetPlayClient("127.0.0.1", netplay_server->GetPort(), npd, std::string(Smashladder::playername.mb_str()) , false,
				config.traversal_host, config.traversal_port);
	}
	else {
		netplay_client =
...
bool NetPlayLauncher::Join(const NetPlayJoinConfig& config)
{
	...
	if (Smashladder::m_netplay) {
		...
		netplay_client =
			new NetPlayClient(host, config.connect_port, npd, std::string(Smashladder::playername.mb_str()) , config.use_traversal,
				config.traversal_host, config.traversal_port);
	}
	if (netplay_client->IsConnected())
	{
